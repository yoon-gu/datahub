...
위 설정이 완료되면 GMS에 요청이 전송될 때 상세한 추적을 볼 수 있어야 합니다. 추적을 더 읽기 쉽게 만들기 위해 여러 곳에 `@WithSpan` 어노테이션을 추가했습니다. 선택한 추적 수집기에서 추적을 보기 시작해야 합니다. 예시 [docker-compose](../../docker/monitoring/docker-compose.monitoring.yml)는 포트 16686에 Jaeger 인스턴스를 배포합니다. `http://localhost:16686` 에서 추적을 볼 수 있어야 합니다.
...
JVM 대시보드에서 CPU/메모리/디스크 사용량과 같은 JVM 메트릭을 기반으로 한 자세한 차트를 찾을 수 있습니다. DataHub 대시보드에서 각 엔드포인트와 Kafka 토픽을 모니터링하는 차트를 찾을 수 있습니다. 예시 구현을 사용하여 `http://localhost:3001` 로 이동하여 Grafana 대시보드를 찾으세요! (사용자명: admin, 비밀번호: admin)
